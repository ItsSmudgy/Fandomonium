...
namespace Fandomonium.Content.NPCs.SpiritOfLight.Projectiles
{
    public class GemBomb : ModProjectile
    {
		...
		public override void AI()
        {
			...
			/*if (Projectile.ai[0] == Projectile.ai[1] - 30)
            {
				for (int i = 0; i < 6; i++)
				{
					...
					FandomoniumUtils.NewProjectileBetter(Projectile.Center, vel, ModContent.ProjectileType<GemBombDeathray>(), (int)(Projectile.damage * 0.65f), 0f, default, dir.ToRotation(), Projectile.whoAmI);
				}
			}*/
        }
		...
		public override void Kill(int timeLeft)
        {
			...
			FandomoniumUtils.NewProjectileBetter(Projectile.Center, Vector2.Zero, ModContent.ProjectileType<GemBombExplosion>(), Projectile.damage * (int)1.25f, 0f);
		}
