...
namespace Fandomonium.Content.NPCs.SpiritOfLight.Projectiles
{
    ...
    {
        public override string Texture => "Fandomonium/Content/NPCs/SpiritOfLight/Projectiles/GemBomb";
            ...
            if (Projectile.localAI[0] == 0f)
            {
                ...
                if (Projectile.ai[1] >= 120 && Projectile.ai[1] % 30 == 0)
                {
                    ...
                    for (int i = 0; i < projCount; i++)
                    {
                        ...
                        FandomoniumUtils.NewProjectileBetter(Projectile.Center, new Vector2(num + num2 * (float)i + vel, speedY), ModContent.ProjectileType<RadiantGemShard>(), Projectile.damage, Projectile.knockBack, ai1: Projectile.whoAmI);
                    }
                }
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                if (Projectile.ai[1] == 560)
                                {
                                    FandomoniumUtils.NewProjectileBetter(Projectile.Center, Vector2.UnitX.RotatedBy(Projectile.localAI[1] + offset), type,
