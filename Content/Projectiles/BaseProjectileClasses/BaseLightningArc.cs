...
namespace Fandomonium.Content.Projectiles.BaseProjectileClasses
{
    public abstract class BaseLightningArc : ModProjectile
    {
		...
		public override void AI()
		{
			...
			if (Projectile.velocity == Vector2.Zero)
			{
				...
				if (Main.rand.NextBool(Projectile.extraUpdates)&& (Projectile.velocity != Vector2.Zero || Main.rand.NextBool((Projectile.localAI[1] == 2f) ? 2 : 6)))
				{
					for (int num885 = 0; num885 < 2; num885++)
					{
						float num886 = Projectile.rotation + ((Main.rand.NextBool(2)) ? (-1f) : 1f) * ((float)Math.PI / 2f);
						...
						int num888 = Dust.NewDust(Projectile.Center, 0, 0, DustID.Electric, vector111.X, vector111.Y);
					...
					if (Main.rand.NextBool(5))
					{
						...
						int num889 = Dust.NewDust(Projectile.Center + vector112 - Vector2.One * 4f, 8, 8, DustID.Smoke, 0f, 0f, 100, default(Microsoft.Xna.Framework.Color), 1.5f);
			...
			else
			{
				...
				if (Splits)
                {
					if (Main.rand.NextBool(4)&& Main.netMode != NetmodeID.MultiplayerClient && Projectile.localAI[1] == 0f)
					{
